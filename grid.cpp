...
void Grid::Run()
{
    ...
    for(int i = 0; i < size; i++)
    {
        for(int j = 0; j < size; j++)
        {
            if(grid[i][j] != NULL && grid[i][j]->Face() != 'D' &&(grid[i][j]->Finished() == true))
            {
